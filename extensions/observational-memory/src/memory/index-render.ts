...
/** Full git-friendly snapshot of the durable memory state. */
export function renderMemorySnapshot(topics: Topic[], topicBodies: Map<string, string>, journey?: string): string {
	const parts: string[] = ["# Observational memory", "", "Generated mirror of the SQLite memory backend. Do not edit by hand.", ""];
	if (journey?.trim()) {
		parts.push("## Journey", "", journey.trim(), "");
	}
	if (topics.length === 0) {
		parts.push("## Topics", "", "_No topics yet._", "");
		return `${parts.join("\n").trimEnd()}\n`;
	}
	parts.push("## Topics", "");
	for (const topic of topics) {
		const updated = topic.updated ? ` — updated ${topic.updated}` : "";
		parts.push(`### ${titleOf(topic)}${updated}`, "", `Path: \`${topic.path}\``, "", topicBodies.get(topic.filename)?.trim() || summaryOf(topic), "");
	}
	return `${parts.join("\n").trimEnd()}\n`;
}
