...
import { readSqliteRunCost, readSqliteRunResult, sqliteHasScope, writeSqliteRunCost, writeSqliteRunResult } from "../memory/sqlite.js";
...
export function writeWorkerCost(path: string, cost: WorkerCostResult): void {
	const root = process.env.OM_MEMORY_DIR;
	if (process.env.OM_MEMORY_BACKEND === "sqlite" && root && process.env.OM_RUN_ID) {
		writeSqliteRunCost(root, process.env.OM_RUN_ID, process.env.OM_WORKER ?? "worker", cost.costUsd);
		return;
	}
...
export function readWorkerCost(path: string): WorkerCostResult | undefined {
	const root = process.env.OM_MEMORY_DIR ?? resolve(dirname(path), "..");
	const runId = path.match(/([^/\\]+)\.cost\.json$/)?.[1];
	if (root && runId && sqliteHasScope(root)) {
		const costUsd = readSqliteRunCost(root, runId);
...
export function readObserverResult(path: string): ObserverRunResult {
	const root = process.env.OM_MEMORY_DIR ?? resolve(dirname(path), "..");
	const runId = path.match(/([^/\\]+)\.result\.json$/)?.[1];
	const sqlite = root && runId && sqliteHasScope(root) ? readSqliteRunResult(root, runId) : undefined;
	const raw = sqlite ?? (JSON.parse(readFileSync(path, "utf-8")) as unknown);
...
export function writeObserverResult(path: string, result: ObserverRunResult): void {
	const root = process.env.OM_MEMORY_DIR;
	if (process.env.OM_MEMORY_BACKEND === "sqlite" && root && process.env.OM_RUN_ID) {
		writeSqliteRunResult(root, process.env.OM_RUN_ID, process.env.OM_WORKER ?? "observer", result);
		return;
	}
