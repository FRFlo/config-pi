...
export default function (pi: ExtensionAPI) {
	...
	pi.registerFlag("bash-guard-enabled", {
		description: "Start the session with bash-guard interactive prompts enabled (hard-block floor still applies when disabled).",
		type: "boolean",
		default: false,
	});
	...
	function showDisabledStatus(ctx: any): void {
		if (!ctx.hasUI) return;
		const { theme } = ctx.ui;
	...
	pi.on("session_start", async (event, ctx) => {
		if (event.reason === "startup" && pi.getFlag("--bash-guard-enabled") === true) {
			disabled = false;
			if (ctx.hasUI) ctx.ui.setStatus(BASH_GUARD_STATUS_KEY, undefined);
			return;
		}
	...
	pi.registerCommand("bash-guard", {
		description: "Toggle bash-guard between disabled (default) and interactive prompts for this session.",
